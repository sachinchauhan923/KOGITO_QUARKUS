...
public class PersonsResource {
    ...
    public Response createResource_persons(@Context HttpHeaders httpHeaders, @Context UriInfo uriInfo, @QueryParam("businessKey") @DefaultValue("") String businessKey, @javax.validation.Valid() @javax.validation.constraints.NotNull() PersonsModelInput resource) {
        ProcessInstance<PersonsModel> pi = processService.createProcessInstance(process, businessKey, Optional.ofNullable(resource).orElse(new PersonsModelInput()).toModel(), httpHeaders.getRequestHeaders(), httpHeaders.getHeaderString("X-KOGITO-StartFromNode"));
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(summary = "persons", description = "")
    public PersonsModelOutput updateModelPartial_persons(@PathParam("id") String id, PersonsModel resource) {
    ...
    public List<TaskModel> getTasks_persons(@PathParam("id") String id, @QueryParam("user") final String user, @QueryParam("group") final List<String> groups) {
        return processService.getTasks(process, id, SecurityPolicy.of(IdentityProviders.of(user, groups))).orElseThrow(NotFoundException::new).stream().map(org.acme.travels.Persons_TaskModelFactory::from).collect(Collectors.toList());
    }
    ...
    @Path("/{id}/ChildrenHandling/{taskId}")
    @Consumes(MediaType.APPLICATION_JSON)
    public org.acme.travels.Persons_5_TaskOutput saveTask_ChildrenHandling_0(@PathParam("id") final String id, @PathParam("taskId") final String taskId, @QueryParam("user") final String user, @QueryParam("group") final List<String> groups, final org.acme.travels.Persons_5_TaskOutput model) {
        return processService.saveTask(process, id, taskId, SecurityPolicy.of(user, groups), model, org.acme.travels.Persons_5_TaskOutput::fromMap).orElseThrow(NotFoundException::new);
    }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public PersonsModelOutput taskTransition_ChildrenHandling_0(@PathParam("id") final String id, @PathParam("taskId") final String taskId, @PathParam("phase") final String phase, @QueryParam("user") final String user, @QueryParam("group") final List<String> groups, final org.acme.travels.Persons_5_TaskOutput model) {
        return processService.taskTransition(process, id, taskId, phase, SecurityPolicy.of(user, groups), model).orElseThrow(NotFoundException::new);
    }
    ...
    @Path("/{id}/ChildrenHandling/{taskId}")
    @Produces(MediaType.APPLICATION_JSON)
    public org.acme.travels.Persons_5_TaskModel getTask_ChildrenHandling_0(@PathParam("id") String id, @PathParam("taskId") String taskId, @QueryParam("user") final String user, @QueryParam("group") final List<String> groups) {
        return processService.getTask(process, id, taskId, SecurityPolicy.of(user, groups), org.acme.travels.Persons_5_TaskModel::from).orElseThrow(NotFoundException::new);
    }
